...
def getPyTable(data):
    ...
    while pos < length:
        ...
        #print l,
        pos += 2
        py = '\''+byte2str(data[pos:pos+l]) #这里可以更改每个字之间的拼音分隔符在前面，现在用的是'号
        #print py
        GPy_Table[index]=py
        pos += l
...
def main():
    if (len(sys.argv) < 3):
        print >>sys.stderr, 'usage: %s <input scel file 1> [input scel file 2 ...] <output scel file>' % sys.argv[0]
        return -1

    output_file = sys.argv[len(sys.argv) - 1]
    for name in sys.argv[1 : len(sys.argv) - 1]:
        deal(name)
...
#        f.write( unicode('{%(count)s}' %{'count':count}+py+' '+ word).encode('GB18030') )#最终保存文件的编码，可以自给改
        f.write( unicode(py+' '+ word).encode('GB18030') )
        f.write('\n')
    f.close()
