...
<big> 二、部件输入方法（精简码）</big>
    ...
    利用"6、7、8、9、0"来代表部件笔画"横一、竖丨、撇丿、点丶、折乛"，
    击下这些键后，将在输入框中显示出以该笔画起笔的8-10个常用部件，用户根据部件的序号击相应数字完成部件输入。
    5个笔画引出的常用部件及序号如下（共48个）：
    
    61 王    62 木朩    63 石    64 艹    65 车車    66 扌    67 土士走    68 酉    69 雨    60 廿革堇
    71 日    72 目    73 山    74 田    75 口    76 ⻊足    77 虫    78 贝貝    79 罒四皿    70 巾
    81 钅釒金    82 亻    83 禾    84 人飠食    85 月    86 ⺮竹    87 犭    88 彳    89 鱼魚    80 夂攵
    91 广    92 忄心    93 火灬    94 氵    95 讠訁言    96 方    97 宀    98 疒    99 礻    90 衤
    01 尸    02 阝卩    03 女    04 又    05 纟糹糸    06 马馬    07 厶幺    08 弓    09（序号）   00（标点符号）
...
<big>三、智能渐进方式</big>
    ...
    因此需要在输入过程中将一些不是很常用的多笔部件作了交互式的智能处理，即：利用"1、2、3、4、5"来拓展部件笔画。
    在输入过程中若在输入框中出现了要输入字的部件（偏旁部首），则这个部件的剩余代码不必再输，转为输它的后续代码。
    以下为智能渐进部件（共35个） :
    
    10  镸髟    11 丰耒（敖字边青字头）  12 其    13 歹    14 覀西    16 邦字边（春字头）    17 耳    18 冓字头（赛字中）
    20 骨    21 虍    22 业    24 肖字头（党字头）    25 黑    26 非    27 門鬥    29 且（直字底）
    30 饣    31 牜牛生    32 臼    33 舟    34 爫豸（受字头）    35 角    36 身    37 舌    38 矢    39 鸟鳥
    40 气    41 立（旁字头）    42 门    43 米    44 学字头（學字头）    46 隹字边    47 亥    48 半羊    49 关（卷判差养字头）
    52 子    54 癶    59 鬼
...
<big>四、盲打输入规则（六全码）</big>
    ...
    按照笔顺输入笔画，再结合部件的输入方式，95%以上的常用汉字都能在4键之内显示出来，
    若要进一步提高输入其它汉字速度，逐步过渡到盲目打高速输入方式，则需掌握六码的取码方法。
    ...
    由于有少部分精简码与六全码雷同，请输入法开发者把精简码所代表的常用汉字排在候选列表靠前的位置。
