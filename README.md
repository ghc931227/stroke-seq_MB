笔顺数码输入法码表 stroke-seq_MB 第一版 v1（2016.09.09）
...
<big>显示说明</big>
    ...
    候选字列表显示在悬浮栏第二行。编码输入完毕，用户选择了所需要的字之后：
    悬浮栏第一行显示刚才这个字的“精简码”、“六全码”，方便用户学习记忆。
    第二行显示最近经常输入的句子或短语，或者联想词组。
...
<big>二、六全码、精简码</big>
    ...
    若该字最后一部分为常用部件，则取该常用部件的首笔编码。例：
    
    党：247535（"党"的最后一笔为"乛"，第六码取5）
    漭：946412（"漭"的最后一笔为"丨"，第六码取2）
    涯：941366（"涯"最后部分为常用部件"土"，第六码取部件"土"的首笔部件笔画编码"6"）
    骞：971806（"骞"最后部分为常用部件"马"，第六码取部件"马"的首笔部件笔画编码"0"）
    回：25751 （"回"字不能拆解为二个“口”，因为按笔顺外面大“口”的最后一笔要等小“口”写完后才能封口）
    再：125211（"再"最后三笔虽然像是一个“土”字，但按笔顺则应该是先一竖再二横，因此不能取“土”作为部件）
    
    六全码目前一共录入了20903个汉字，其中包括了繁体字、异体字、以及日韩所用的汉字。
    精简码则是六全码的简化格式，1~6键即可快速打出最常用的6765个汉字。例：
...
<big>三、部件编码表</big>
    ...
    利用"6、7、8、9、0"来代表部件笔画"横一、竖丨、撇丿、点丶、折乛"，配以0~9按键，形成简单常见的三笔及三笔以上部件。
    ...
    部件编码的排列顺序基本是以汉字笔顺为准，少数部件按思维习惯及视觉习惯来编排，部件的选用标准是汉字起头的边旁部首优先。
    原本就是二个笔画的边旁部首就不再作为基本部件占用此表，因笔顺相近而放在下表拓展部件中，例如： 亻冫讠刂 丷冖人 八 又
    用这二个数字来引出最常用的边旁部首或部件，编码如下（共48个）：
    
    60 土士走    61 王     62 木朩   63 石    64 雨      65 车車   66 扌   67艹⺿艸   68 廿革堇    69 耳
    70 山       71 日曰   72 目     73 田由甲申  74 罒四皿   75 口  76 ⻊足    77 虫      78 贝貝   79 ⺌
    80 夂攵     81 钅釒金  82 禾     83 彳    84 爫豸    85 月     86 ⺮        87 犭    88 鱼魚   89 鸟鳥
    90 疒       91 广     92 忄    93 火灬   94 氵      95 心必   96 立（旁字头） 97 礻   98 衤    99 宀
    00（标点符号）01 弓    02 孑子   03 女    04 幺    05 纟糹   06 尸    07 彐（录肀字头） 08 马馬   09（数字序号）
    
    例如：“月”字第一笔是撇丿即8,第一笔是折乛即5。又如：走之底 辶 大多用在末尾，不是起头，就不列在表内，按笔顺输入即可。
    如："珏"字，"王"为一部件，首笔为"一"，输入6，"王"便出现在提示框中，序号为1，再输入1(也可直接鼠标点击)就完成"王"部件的输入；
    然后输入下一个部件仍是“王”编码61，最后输入"点丶"编码4，候选字出现"珏"字排在第一位，按回车或空格完成输入。"珏"的编码为61614。
    再如："内"字，输入前两笔编码25，"人"是一部件，首笔为"丿"，输入8，"人"出现在提示框中，序号为4，所以输入4。"内"的编码为2584。
    
    利用"1、2、3、4、5"配以0~9按键，来拓展笔画数多、不大常用、比较复杂的部件笔画（共39个）。原本就是二个笔画的边旁部首也在内。
    不需要死记硬背，在输入过程中若在提示框中出现了要输入字的部件（偏旁部首），则这个部件的剩余编码不必再输，转为输它的后续编码。
    
    10 镸髟  11 （敖字边）  12  十其    13  厂歹豕    14 覀西    15 酉   16 丰耒（邦字边）  17 冓字头（赛字中）
    20 骨    21 止歺   22 刂业  23 虍    24 卜非    25 巾（有字底）  26 門鬥    27 且（直字底）
    30 饣飠  31 牜牛   32  亻  33 ⺁舟彡 34 人入  35 勹角（然祭字头） 36 矢  37 片身臼（段舆盥字边）  38 舌   39 鬼
    40 冖    41 冫亠方  42 门   43 丷兰（关字头中）  44 兴字头（学學字头中） 45 讠  46 訁言  47米  48 六文  49 手毛气
    50 巜巛   51 巳    52 阝卩  53 羽    54 又廴厶癶    55 彑毌母   56 艮（即字旁）  57 爿
    
    如，输入"物"字：第一笔：撇笔31，"牜"即出现在提示框中，此时"牜"的剩余编码不必再输，
    转为输"物"的后续编码（即"勿"的编码），刚输入3，"物"就出现在候选汉字的第一位，其精简码为313，六全码为313533。
    再如，输入"鞭"字："廿革"为一常用部件，编码"67"，输完之后，"革"出现在提示框中，则"革"的剩余编码不必再输，
    转为输"鞭"的后续编码（即"亻"的编码），刚输入8，"鞭"出现候选汉字的第一位，其精简码为608，六全码为678214。
...
<big>    六、易错易混汉字</big>
    ...
    以下汉字是容易输入错误或混淆的：
    1、"末、未、果"等后四笔形成的"木"形结构，其结构清晰，易于辨认，且笔画顺序与"木"相同，汉字的意义也与"木"相关，故取"木"的编码62；
    2、"都、教、考、孝"等前三笔形成的"土"形结构，其结构清晰，易于辨认，且笔画顺序与"土"相同，故取"土"的编码60；
    ...
    6、"甘、某"等的开头几笔形成的结构即非"艹"字头，也非"廿"头，需按其笔顺取码122；
    7、"里、垂、重"等最后三笔形成的"土"形结构，因其笔画顺序与"土"字旁不同（先一竖再横），故需按其笔顺取笔画编码；
    8、"鹤、雀"等字中有类似"亻"的结构，均不作为"亻"，需取其笔画编码32；
    9、"衰、蓑"两字中间的"口"形结构，因按笔顺先写"口"中间的一横，打乱了整个"口"的笔顺，故也不按"口"字旁处理，需取其笔画编码2511；
    10、"缶、击、出"等字最后三笔形成"山"形结构，笔顺也与"山"相同，但不易于辨认，故不取"山"字旁，需取笔画编码252；
    11、"制，刺，敝"等的左边均有类似"巾"的结构，笔顺也与"巾"相同，但不易于辨认，故不取"巾"字旁，需取笔画编码252；
