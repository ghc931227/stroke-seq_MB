单手笔顺输入法码表 stroke-seq_MB 第一版 v1
...
<big>按键说明</big>
    ...
    单手笔顺输入法使用键盘数字小键盘为主要输入工具，只用一只单手（右手或左手）即可完成简体繁体文字、词组以及标点符号的输入。
    ...
    建议输入法程序开发者采用混合输入模式，即：小键盘是笔顺输入法，大键盘默认是英文输入法，按一下shift键可切换为拼音输入法。
...
<big>显示说明</big>
    ...
    第二行显示最近该用户经常输入的句子或短语，以供用户选取输入（这很有用），其后还可以跟随有该字的联想词组以供用户快速输入。
