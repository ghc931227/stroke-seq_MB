...
<big>按键说明</big>
    ...
    建议输入法程序开发者采用混合输入模式，即：小键盘是笔顺输入法，大键盘默认是英文输入法，开启大写键时为拼音输入法。
...
<big>显示说明</big>
    ...
    候选字列表显示在悬浮栏第二行（候选框）。建议精简码显示为黑色、六全码显示为85%灰度、笔顺码显示为65%灰度。
    编码输入完毕，用户+号切换到候选字（蓝色）选词模式，三类编码显示为同上三种深度的蓝色，用户选择了所需要的字之后：
    悬浮栏第一行显示刚才这个字、词的“精简码”、“六全码”，及其汉语拼音，方便用户学习记忆。最后有附加按钮可链接字典网站。
    第二行显示最近该用户经常输入的句子、或短语，以供用户选取输入，其后还可以跟随有该字的联想词组以供用户快速输入。
...
<big>二、六全码、精简码</big>
    ...
    精简码则是六全码的简化格式，囊括了《通用规范汉字表》大部分一级、二级汉字，1~5键即可快速打出最常用的汉字。例如：
...
<big>三、部件编码表</big>
    ...
    60 方 61 立（旁商字头中）62 礻 63 衤 64 與𦥯（兴学字头中）65 心必 66 文六亦䜌（变字头）67 手毛气 68 米㡀 69 火灬𤇾
    ...
    精简码位数、精简码该字笔画数、精简码数字大小、六全码字频、六全码该字笔画数、六全码数字大小、笔顺码数字大小、笔顺码字频。
    例如：编码194514、编码13251354，都可以用于编程测试，因为在输入的过程中，汉字会出现上述排列情况。
...
<big>    六、易错易混汉字</big>
    ...
    9、“生”字虽然有类似“牛”字的结构，但是从记忆习惯上通常认为是一撇加“”字结构，因此不取“牛”字31编码，而取“”字71编码。
    10、“满、螨”等有类似“”编码18的部首，但应取“艹”编码17,因为艹字头与下方的一横是断开的，且从习惯上通常认为“一”属于“两”字。
    11、“垂、郵、甀、乗、剰”等字中间有类似“艹”的结构，但是因为混杂在其它笔画之中，不易辨认，故不取17编码，而取笔顺即可。
    12、“脑、恼、垴”的右边有类似“文”字的结构，但是从记忆习惯上通常认为“乂”是属于“凶”字结构，因此不取“文”字66编码，取笔顺即可。
    13、“刪、姍、柵"等字中虽然有类似“卄”字的结构，但是它的笔顺不是与“侖”字那样先一横再二竖，而是先二竖再一横，所以不取17编码。
    14、“刺、棘、枣、策”等字中虽然有类似“巾”字的结构，但是因为混杂在其它笔画之中，不易辨认，故不取25编码，而取笔顺即可。
    15、“夜、腋”等字中虽然有类似“夕”字的结构，但是因为被其它笔画混杂，不易辨认，故不取90编码，而取笔顺即可。
    16、“熏、薰、醺”等字中虽然有类似“黑”字的结构，但因为中间一竖穿透了“黑”字顶部，与上方其它笔画混杂，故不取86编码，而取笔顺。
    17、有些汉字，在不同的电脑字体里显示出来的写法不一样，例如“爋”字的最后四点在有些字体中是放在中间的，这类汉字以国标笔顺为准。
