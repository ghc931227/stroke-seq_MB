...
<big>显示说明</big>
    ...
    用户输入编码时：
    已输入的数字、其所代表的笔画、以及所代表的字首部件显示在悬浮栏第一行（提示框），候选字列表显示在悬浮栏第二行；
    编码输入完毕，用户选择了所需要的字之后：
    悬浮栏第一行显示刚才这个字的“精简码”、“六全码”，第二行显示最近经常输入的句子或短语，或者联想词组。
    输入法悬浮栏应当还提供有其它按钮，方便用户进行相关设置。
...
<big>三、部件编码表</big>
    ...
    利用"6、7、8、9、0"来代表部件笔画"横一、竖丨、撇丿、点丶、折乛"，配以0~9按键，
    用这二个数字来引出最常用的边旁部首或部件，编码如下（共48个）：
    
    60 土士走    61 王     62 木朩   63 石    64 扌      65 车車     66 艹⺿艸       67 廿革堇  68 耳    69 雨
    70 山       71 日     72 目     73 田    74 罒四皿   75 口      76 ⻊足         77 虫      78 贝貝   79 巾
    80 夂攵     81 钅釒金  82 亻     83 彳    84 人飠食   85 月      86 ⺮竹         87 犭      88 禾     89 鱼魚
    90 广       91 疒     92 忄心   93 火灬   94 氵      95 讠訁言   96 立（旁字头中） 97 宀      98 礻    99 衤
    00（标点符号）01 尸     02 阝卩   03 女    04 厶幺     05 纟糹糸   06 马馬         07 孑子    08 弓    09（数字序号）
    
    部件编码的排列顺序基本是以汉字笔顺为准，少数部件按思维习惯及视觉习惯来编排。例如：“月”字第一笔是撇丿即8,第一笔是折乛即5。
    ...
    利用"1、2、3、4、5"配以0~9按键，来拓展笔画数多、不大常用、比较复杂的部件笔画（共33个）。
    在输入过程中若在提示框中出现了要输入字的部件（偏旁部首），则这个部件的剩余编码不必再输，转为输它的后续编码。
    由于某些部件没有相应的单个汉字，在此以“某字边、某字头”来说明，请输入法开发者在制作提示框时，改为用图片显示正确的部件形式。
    
    10 镸髟  11 丰耒（敖字边）  12 其     13 歹豕    14 覀西    15 酉   16 （邦字边）  18 冓字头（赛字中）
    20 骨    21 虍      22 且（直字底）    24 ⺌    25 黑      27 門鬥
    30 饣    31 牜牛生  32 臼（段舆盥字边）  33 ⺁舟    34 爫豸（受字头中）  35 角（祭字头）36 矢  37 身  38 舌  39 鸟鳥
    40 气    41 方      42 门    43 米    44 学字头中（學字头中） 45 兰（关字头中）
    54 癶    59 鬼
    
    如，输入"物"字：
    第一笔：撇笔31，"牜"即出现在提示框中，此时"牜"的剩余编码不必再输，
    转为输"物"的后续编码（即"勿"的编码），刚输入3，"物"就出现在候选汉字的第一位，其精简码为313，六全码为313533。
    
    再如，输入"鞭"字：
    "廿革"为一常用部件，编码"67"，输完之后，"革"出现在提示框中，则"革"的剩余编码不必再输，
